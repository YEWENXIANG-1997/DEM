...
for c in con_d:
    ...
    if c[0] == "threshold":
        fn_threshold = c[1]
        # threshold_overlap = float(c[1])

threshold_d = np.genfromtxt(
    fn_threshold, delimiter=",", skip_header=1)
...
packing = packingRunnable(
    ...
    R=R,
    H=H,
    threshold_overlap=threshold_d,
    incT_snap=incT_snap,
    iniFac=initialfactor,
    prad=r_h
)
